...
const UpdateInterviewer = ({route, navigation}) => {
  ...
  useEffect(() => {
  //if route is not null. Then sert the datas
  setEmployeeId(otherParam.employeeid)
  setFirstName(otherParam.firstName)
  setGender(otherParam.gender)
  setEmailId(otherParam.emailId)
  setMobileNo(otherParam.mobileNo)
  console.log('====================================UPDATE');
  console.log(JSON.stringify(otherParam.skills));
  console.log('====================================UPDATE');
  ...
  const goHome = (id, user, skillList) => {
    const interviewerData = {id: id, user: user, skillList: skillList};
    {
      ...
      axios
        .post(
          BASE_URL + 'management/UpdateInterviewPanelUser',
          interviewerData,
          addinterviewConfig,
        )
        .then(response => {
